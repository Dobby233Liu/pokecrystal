...
.room_in_party
	...
	ld [wWildMon], a
	ld a, [wBattleType]
	cp BATTLETYPE_CONTEST ; fuck, lavender town is park
...
.caught
	...
	jr nz, .load_data ; yes if you transform, you're just a ditto
...
; Reality: no boost
	...
	ld a, BANK("Evolutions and Attacks")
	call GetFarByte
	cp MOON_STONE ; why do you want to use Red's BURN_HEAL
	pop bc
	ret nz
...
.loop
	...
	jr z, .next
	cp c
	jr nz, .loop ; why you'll want only 3 to boost
	sla b
	jr c, .max
